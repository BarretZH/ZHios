...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        [self.cancelBtn setTitle:NSLocalizedString(@"EDIT_PROFILE_PAGE_CANCEL", nil) forState:UIControlStateNormal];
        ...
        [self.conpleteBtn setTitle:NSLocalizedString(@"EDIT_PROFILE_PAGE_DONE", nil) forState:UIControlStateNormal];
...
- (void)setArrayType:(ARRAYTYPE)arrayType
{
    ...
    switch (arrayType) {
        ...
        case DeteArray:
        {
            self.selectLb.text = NSLocalizedString(@"EDIT_PROFILE_PAGE_DATE_OF_BIRTH", nil);
            [self creatDate];
        }
            break;
...
- (void)creatDate{
    ...
    NSString *currentYear = [NSString stringWithFormat:@"%d",abc-5];
    [self.pickerV selectRow:[yearArray indexOfObject:currentYear]+(abc-5-1916+1)*50 inComponent:0 animated:YES];
    
    [formatter setDateFormat:@"MM"];
    NSString *currentMonth = [NSString stringWithFormat:@"%02d",[[formatter stringFromDate:date]intValue]];
    [self.pickerV selectRow:[monthArray indexOfObject:currentMonth]+12*50 inComponent:1 animated:YES];
    
    [formatter setDateFormat:@"dd"];
    NSString *currentDay = [NSString stringWithFormat:@"%02d",[[formatter stringFromDate:date]intValue]];
...
//            NSString *str = [arr objectAtIndex:[self.pickerV selectedRowInComponent:i]];
            NSString * number = [NSString stringWithFormat:@"%ld",(long)[self.pickerV selectedRowInComponent:i]];
            fullStr = [fullStr stringByAppendingString:number];
        }
